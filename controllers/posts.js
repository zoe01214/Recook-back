...
export const delImage = async (req, res) => {
  if (req.user.role !== 1) {
    res.status(403).send({ success: false, message: '沒有權限更改文章' })
  }
  ...
  try {
    ...
    const result = await posts.findByIdAndUpdate(req.params.id, data, { new: true })
    res.status(200).send({ success: true, message: '文章修改成功', result })
  } catch (error) {
    console.log(error.code)
    if (error.name === 'ValidationError') {
      const key = Object.keys(error.errors)[0]
      ...
      res.status(400).send({ success: false, message: '文章已存在' })
    } else {
      res.status(500).send({ success: false, message: '伺服器錯誤' })
    }
  }
